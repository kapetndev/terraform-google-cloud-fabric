...
| <a name="input_display_name"></a> [display\_name](#input\_display\_name) | Fully qualified, authoritative display name of the project. When set, `name` and `project_id` are mutually exclusive. Set one or the other, not both. | `string` | `null` | no |
...
| <a name="input_name"></a> [name](#input\_name) | The project name. Used as a prefix when generating the project ID unless `project_id` is explicitly set. | `string` | `""` | no |
