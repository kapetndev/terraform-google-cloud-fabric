...
resource "google_project" "project" {
  ...
  name                = coalesce(var.display_name, local.name)
  org_id              = local.parent_type == "organizations" ? local.parent_id : null
  project_id          = local.project_id

  lifecycle {
    precondition {
      condition     = var.display_name == null || !(var.name != "" && var.project_id != null)
      error_message = "name: when `display_name` is set, `name` and `project_id` are mutually exclusive. Set one or the other, not both."
    }
  }
}
