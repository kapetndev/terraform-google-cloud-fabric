...
variable "name" {
  description = "Name of the node pool. Used as a prefix for the generated pool name unless `override_name` is set."
  type        = string
  default     = null
}
...
variable "prefix" {
  description = "An optional prefix prepended to `name` when generating the node pool name. Has no effect when `override_name` is set. Cannot be an empty string — use null to omit."
  type        = string
  default     = null
  validation {
