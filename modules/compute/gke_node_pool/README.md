...
| <a name="input_name"></a> [name](#input\_name) | Name of the node pool. Used as a prefix for the generated pool name unless `override_name` is set. | `string` | `null` | no |
...
| <a name="input_override_name"></a> [override\_name](#input\_override\_name) | Fully qualified, authoritative name of the node pool. When set, `name` is ignored and this value is used directly with no suffix appended. | `string` | `null` | no |
| <a name="input_prefix"></a> [prefix](#input\_prefix) | An optional prefix prepended to `name` when generating the node pool name. Has no effect when `override_name` is set. Cannot be an empty string — use null to omit. | `string` | `null` | no |
