...
(Required) recurring_window - Time window for recurring maintenance operations.
  (Required) end_time - Time for the (initial) recurring maintenance to end in RFC3339 format. This value is also used to calculte duration of the maintenance window.
  (Required) start_time - Time for the (initial) recurring maintenance to start in RFC3339 format.
  (Optional) recurrence - RRULE recurrence rule for the recurring maintenance window specified in RFC5545 format. This value is used to compute the start time of subsequent windows.
...
(Optional) exclusions - Exceptions to maintenance window. Non-emergency maintenance should not occur in these windows. A cluster can have up to three maintenance exclusions at a time.
  (Required) end_time - Time for the maintenance exclusion to end in RFC3339 format.
  (Required) name - Human-readable description of the maintenance exclusion. This field is for display purposes only.
  (Required) start_time - Time for the maintenance exclusion to start in RFC3339 format.
  (Optional) scope - The scope of the maintenance exclusion. Possible values are `NO_UPGRADES`, `NO_MINOR_UPGRADES`, and `NO_MINOR_OR_NODE_UPGRADES`.
EOF
  type = object({
...
null, access is unrestricted beyond the gcp_public_cidr_access_enabled flag.
For private clusters this should always be set to known egress CIDRs such as
office, VPN, or CI/CD runner IP ranges.
...
variable "name" {
  description = "Name of the cluster. Used as a prefix for the generated cluster name unless `descriptive_name` is set."
  type        = string
}
...
variable "prefix" {
  description = "An optional prefix prepended to `name` when generating the cluster name. Has no effect when `descriptive_name` is set. Cannot be an empty string — use null to omit."
  type        = string
  default     = null
  validation {
...
(Optional) enable_private_endpoint - Expose the master via its internal IP only, removing all public access to the API endpoint. Defaults to false. When true, `master_authorized_networks` must include your management CIDRs.
...
variable "security_group" {
  description = "Google Groups security group name for Kubernetes RBAC integration. Must be in the format `gke-security-groups@yourdomain.com`. See https://cloud.google.com/kubernetes-engine/docs/how-to/google-groups-rbac."
  type        = string
  default     = null
}
