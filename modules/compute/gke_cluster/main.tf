...
resource "google_container_cluster" "kubernetes_cluster" {
  ...
  # Master endpoint access controls. The block must be absent entirely when
  # neither authorised networks nor the GCP access flag is configured, so a
  # dynamic block is used. When present, access is restricted to the provided
  # CIDR blocks with GCP public ranges optionally included.
  dynamic "master_authorized_networks_config" {
    for_each = var.master_authorized_networks != null || var.enable_gcp_public_access != null ? [""] : []
  ...
  # Enforce mutual exclusivity of release_channel and kubernetes_version. A
  # check block would only warn — a precondition here errors at plan time.
  lifecycle {
    precondition {
      condition     = (var.kubernetes_release_channel == null) != (var.kubernetes_version == null)
      error_message = "Exactly one of 'kubernetes_release_channel' (recommended) or 'kubernetes_version' must be set. Set the other to null."
    }
    precondition {
      condition     = !var.private_cluster.enable_private_nodes || var.ip_allocation_policy != null
      error_message = "ip_allocation_policy must be set when enable_private_nodes is true. Private clusters require VPC-native networking."
    }
  }
}
