locals {
  ...
  # The cluster name is either the caller-specified override or a generated
  # random name.
  cluster_name = var.override_name != null ? var.override_name : random_id.cluster_name[0].hex
...
resource "google_container_cluster" "kubernetes_cluster" {
  ...
  # L4 Internal Load Balancer (ILB) Subsetting allows GKE to automatically
  # remove unhealthy nodes from ILB backend pools, improving availability and
  # performance for services using L4 ILBs. This is a recommended setting for
  # production clusters.
