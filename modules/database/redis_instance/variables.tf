...
variable "maintenance_config" {
  description = <<EOF
Maintenance window configuration. When null, Google may perform maintenance at any time.
...
EOF
  ...
  validation {
    ...
    error_message = "maintenance_config: `maintenance_window.day` must be between 1 and 7 and `maintenance_window.hour` must be between 0 and 23."
  }
}
...
variable "name" {
  description = "Name of the instance. Used as a prefix for the generated instance name unless `descriptive_name` is set."
  type        = string
  default     = null
}

variable "prefix" {
  description = "An optional prefix prepended to `name` when generating the instance name. Has no effect when `descriptive_name` is set. Cannot be an empty string — use null to omit."
  type        = string
  default     = null
  validation {
